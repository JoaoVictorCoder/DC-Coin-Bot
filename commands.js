  ...
  client.once('ready', async () => {
    try {
      ...
      for (const guild of client.guilds.cache.values()) {
        ...
        try {
          console.log(`🔄 Registrando comandos em guild ${guild.name} (${guildId})...`);
          await rest.put(
            Routes.applicationGuildCommands(clientId, guildId),
            { body: commands }
          );
          console.log(`✅ Comandos registrados em guild ${guild.name} (${guildId}).`);
        } catch (err) {
          console.error(`❌ Erro ao registrar em guild ${guild.name} (${guildId}):`, err);
        }
      }
    } catch (err) {
